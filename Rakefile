...
task :start, :config do |t, args|
  config = args[:config]
  system("/usr/bin/env JOB_CONFIG=#{config} bundle exec god -c etc/omf_slice_authority.god")
  system('/usr/bin/env bundle exec god start slice_authority')
end

desc "Stop the Service Daemon"
task :stop do |t, args|
  system('/usr/bin/env bundle exec god stop slice_authority')
end

desc "Print the status of the Service daemon"
task :status do |t, args|
  system('/usr/bin/env bundle exec god status slice_authority')
end

desc "Run the  Service in this shell"
task :run do |t, args|
  system("#{TOP_DIR}/bin/omf_slice_authority start")
end
...
task 'create_server_bin' do
  target = 'bin/omf_slice_authority'
