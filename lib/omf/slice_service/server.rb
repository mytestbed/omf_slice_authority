...
require 'omf/slice_service/version'

require "em-xmlrpc-client"
#XMLRPC::Config::ENABLE_NIL_PARSER = true # creates warnings
...
module OMF::SliceService
  ...
  class Server
    ...
    ETC_DIR = File.join(File.dirname(__FILE__), '/../../../etc/omf-slice-service')

    def init_logger(options)
      OMF::Base::Loggable.init_log 'server', :searchPath => File.join(File.dirname(__FILE__), 'server')
      info "Slice Service V #{OMF::SliceService.version}"
    end
    ...
    def load_test_state2(options)
      ...
      s1 = OMF::SliceService::Resource::Slice.create(name: 'slice1',
                                        uuid: s1_uuid,
      ...
      u1 = OMF::SliceService::Resource::User.create(name: 'user1',
                                                    uuid: '22ccd749-9b67-41ee-9de2-afee6ad0457c',
                                                    urn: 'urn:publicid:IDN+ch.geni.net+user+maxott')

      # s1.slice_members << OMF::SliceService::Resource::SliceMember.create(name: 'member1',
                                        # role: 'ADMIN')

      OMF::SliceService::Resource::SliceMember.create(user: u1, slice: s1, role: 'ADMIN')

      # s2 = OMF::SliceService::Resource::Slice.create(name: 'slice2',
                                        # urn: 'default_slice',
                                        # expiration: Time.now + 86400,
                                        # email: 'mary@some.edu',
                                        # description: 'Mary\'s slice')
      # s3 = OMF::SliceService::Resource::Slice.create(name: 'slice3',
                                        # urn: 'default_slice',
                                        # expiration: Time.now + 86400)
    ...
    def read_config_file(o)
      ...
      f = File.basename(cf)
      d = File.dirname(cf)
      config = (OMF::Base::YAML.load(f, :path => [d])[:slice_service]) || {}
    ...
    def run(opts = DEF_OPTS, argv = ARGV)
      opts[:handlers] = {
        ...
        :pre_parse => lambda do |p, options|
          ...
          p.separator ""
          p.separator "Slice Service options:"
          p.on("--config FILE", "Slice Service config file [#{options[:config_file]}]") do |n| options[:config_file] = n end
          p.separator ""
